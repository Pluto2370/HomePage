...
function renderBookmarks() {
    ...
    bookmarksData.forEach(function(item, index) {
        html += '<div class="bookmark-item" data-index="' + index + '" draggable="true">' +
...
function showAddForm(addItem) {
    ...
    form.style.top = rect.top - 160 + 'px';
...
function editCurrentBookmark() {
    ...
    document.getElementById('bookmarkIcon').value = item.icon !== item.name.charAt(0) ? item.icon : '';
    closeEditForm();
...
function confirmAdd() {
    ...
    var customIcon = document.getElementById('bookmarkIcon').value.trim();
    if (!name || !url) return;
    var icon = customIcon || name.charAt(0);
